...
export function splitArrayOnce<T>(arr: T[]): [T[], T[]] {
    ...
    const firstsplit = arr.slice(0, split)
    const secondsplit = arr.slice(split, arr.length)

     myarr[0].push(...firstsplit)
     myarr[1].push(...secondsplit)
...
export function splitArrayTwice<T>(arr: T[]): [[T[], T[]], [T[], T[]]] {
    ...
    const xfirstsplit = splitArrayOnce(firstsplit)
    const xsecondsplit = splitArrayOnce(secondsplit)

    
    let myarr: [[T[], T[]],[T[], T[]]] = [xfirstsplit, xsecondsplit]
    return myarr;
}
...
export function splitArray<T>(arr: T[]): NestedArray<T> {
    ...
    let NA: NestedArray<T>;
    if( arr.length == 0) {
        return NA = mkNALeaf()
    }
    let splitarr: [T[], T[]] = splitArrayOnce(arr)  
    let current = splitarr[0][splitarr[0].length - 1]
    let arr1 =  splitArray(splitarr[0].slice(0,-1))
    let arr2 = splitArray(splitarr[1])

      NA = mkNANode(current,arr1,arr2)
    return NA 
...
export function heightNaryTree<T>(naTr: NaryTree<T>): number {
   ...
   switch(naTr.tag) {
       ...
       case "NODE": {
           ...
           return 1 + Math.max(heightNaryTree(naTr.firstChild), naTr.restChildren.length)
       }
   }
}
...
export function mapNaryTree<T, U>(naTr: NaryTree<T>, f: (arg: T) => U): NaryTree<U> {
    ...
    switch(naTr.tag) {
        ...
        case "NODE": {
            ...
            arr.push(mapNaryTree(naTr.firstChild,f))
            naTr.restChildren.map(x => arr.push(mapNaryTree(x,f)))
            return mkNaryNode(f(naTr.contents), arr)
        }
    }
...
export function nestedArrayToNaryTree<T>(na: NestedArray<T>): NaryTree<T> {
    ...
    switch(na.tag) {
        ...
        case "NODE": {
           arr.push(nestedArrayToNaryTree(na.left))
           arr.push(nestedArrayToNaryTree(na.right))
           return mkNaryNode(na.contents, arr)
        }
    }
...
console.log(JSON.stringify(nestedArrayToNaryTree(tr3)))
